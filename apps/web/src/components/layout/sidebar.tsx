...
export default function Sidebar({ role }: SidebarProps) {
    ...
    const navigation = [
        ...
        { name: "내 티켓", href: "/tickets?filter=me", icon: TicketIcon },
        // Only show admin tools if user is ADMIN
        ...(role === "ADMIN" ? [
            { name: "전체 티켓", href: "/admin/tickets", icon: TicketIcon },
        ...
        <div className="flex flex-col flex-grow border-r bg-white dark:bg-slate-900 pt-5 pb-4 overflow-y-auto w-full h-full shadow-sm">
            ...
            <div className="px-4 pb-2 border-t pt-4">
                <div className="flex items-center text-sm font-medium text-slate-500 dark:text-slate-400">
                    권한: <span className="ml-2 badge bg-primary/10 text-primary px-2 py-1 rounded-sm text-xs font-bold">{role}</span>
                </div>
            </div>
        </div>
    );
